...
function fillInfoWindow(marker, infowindow) {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            infowindow.setContent('<b>' + marker.title + '</b><br><div>Foursquare Link:</div><div><a href="' + url + '">' + hotelAddress + '</a></div><div id="foursquare-time">Timings: </div>');
            ...
            $.ajax({
                ...
                success: function(request1) {
                    ...
                    var hotelTime1 = request1.response.popular.timeframes !== undefined ? request1.response.popular.timeframes[1].open[0].start : "No timings available";
                    var hotelTime2 = request1.response.popular.timeframes !== undefined ? request1.response.popular.timeframes[1].open[0].end : " ";
                    var hotelTime3 = request1.response.popular.timeframes !== undefined ? request1.response.popular.timeframes[1].open[1].start : " ";
                    var hotelTime4 = request1.response.popular.timeframes !== undefined ? request1.response.popular.timeframes[1].open[1].end : " ";
                    var allTimings;
                    if (hotelTime1 === "No timings available" && hotelTime2 === " " && hotelTime3 === " " && hotelTime4 === " ") {
                        allTimings = hotelTime1;
                    } else {
                        allTimings = (hotelTime1 + "-" + hotelTime2 + ", " + hotelTime3 + "-" + hotelTime4);
                    }
                    //Populate the returned JSON hours to respective marker's infowindow
                    infowindow.setContent('<b>' + marker.title + '</b><br><div>Foursquare Link:</div><div><a href="' + url + '">' + hotelAddress + '</a></div><div>Timings:' + allTimings + ' </div>');
                },
...
//On successfull callback for google map API
function successMAP() {
